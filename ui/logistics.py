...
class LogisticsManager(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        for col in ("Name", "Language", "Phone"):
            ...
            self.guides_tree.column(col, width=100)
        ...
        add_guide_btn = ttk.Button(guides_frame, text="Добавить гида", command=self.add_guide_popup)
        add_guide_btn.pack(pady=5)
        ...
        add_transfer_btn = ttk.Button(transfers_frame, text="Добавить трансфер", command=self.add_transfer_popup)
        add_transfer_btn.pack(pady=5)

    def load_guides(self):
        for row in self.guides_tree.get_children():
            self.guides_tree.delete(row)
    ...
    def load_tours(self):
        ...
        self.tours = {f"{row[1]}, {row[2]} ({row[3]} - {row[4]}) (ID:{row[0]})": row[0] for row in cursor.fetchall()}
    ...
    def add_transfer_popup(self):
        ...
        tour_cb.grid(row=0, column=1, padx=5, pady=5)
